...
const reFIX = /-(fe0f|fe0e|1f3fb|1f3fc|1f3fd|1f3fe|1f3ff)/g;
...
function createEmojiImage(span, emoji, cp) {
    ...
    createImage(src, { alt: emoji, draggable: false }).then(
        img => {
...
function createEmoji(emoji) {
    ...
    if (!EXCLUDE.includes(name)) {
        if (isTextStyle) {
            ...
            span.classList.add('x-emoji-color');
            if (GM_getValue('use-emoji-image'))
...
mutationObserver(() => {
    ...
    // Replace the shitty YouTube emojis. 💩
