...
const reUFE0F = /\uFE0F/gu; // Variation Selector-16 (emoji-style)

// Fitzpatrick scale (1F3FB-1F3FF).
// VS-15, VS-16, and skin tone variations modifiers.
const reFIX = /-(fe0f|fe0e|1f3fb|1f3fc|1f3fd|1f3fe|1f3ff)/g;

// Unicode Emoji 15.1.
// Modified to include the Variation Selector-15 (FE0E).
...
for (let key in emojis) {
    const name = emojis[key];
    delete emojis[key];
    emojis[key.replace(reFIX, '')] = name;
}
...
function createEmoji(emoji) {
    ...
    setTimeout(() => {
        ...
        if (!span.closest('[title]')) {
            const cp2 = cp.replace(reFIX, '');
            const name = emojis[cp2] || emojis2[cp2];
            if (!name) console.warn('Emoji name not found:', span);
...
function parseTextNode(node) {
    ...
    )) node.replaceWith(createSpan(fragment, 'x-emoji-wrapper'));
}
